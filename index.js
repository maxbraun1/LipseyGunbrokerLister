...
function postOnGunBroker(item){
  return new Promise( async (resolve, reject) => {
    ...
    try{
      ...
      logProcess("Checking if item is already posted...");
      ...
      logProcess("Generating Images...");
      await generateImages("https://www.lipseyscloud.com/images/"+item.imageName).catch((error) => {
        reject(error);
        return;
      });
      console.log("here17");
...
function deleteTmpFiles(){
  logProcess("Deleting Posted Images...");
...
async function postAllListings(listings, limit){
  ...
  for(let item of listings){
    count++;
    console.log(chalk.black.bgBlue.bold(" Listing number ["+count+"] "));
    await postOnGunBroker(item).catch((error) => console.log(error));
  }
}

function descriptionGenerator(item){
    let html;
    html = html + "<div style='padding:0;margin:0;background-color:#ffffff;border:1px solid gainsboro;color:black;font-family:Arial, Helvetica, sans-serif;margin:0px;padding:30px;'>"
    ...
    html = html + "<div style='width: 100%;background-color: #6de086;padding: 10px;display: flex;border-radius: 5px;border: 2px solid #5cbe71;box-sizing:border-box;'>"
    html = html + "<img style='width:25px;height:25px;' src='https://seattleengrave.wpengine.com/wp-content/uploads/2022/12/california.png'/>"
    html = html + "<p style='line-height:25px;font-weight:bold;text-transform:uppercase;margin-bottom:0; margin-left:10px; margin-right:0; margin-top:0;'>California Compliant</p>"
    html = html + "</div>"

    html = html + "<hr style='border:1px solid gainsboro;margin:20px 0;'/>"

    html = html + "<h2 style='line-height:1.4em;'>Ruger Security 9 Compact 9mm 3.42&#39;&#39; 10-Rd Pistol</h2>"
    html = html + "<p style='background-color:gainsboro;color:black;width:fit-content;padding:7px;border-radius:3px;'><strong>UPC:</strong> 736676038374</p>"
    
    html = html + "<p style='color:black;margin:20px 0;'>The Ruger Security-9 Compact Semi-Auto Pistol conceals easily and carries well. The Security-9 Compact operates with the same Secure Action used in the LCP II&reg;, which provides a smooth trigger pull with an internal hammer; the trigger pull feels short and crisp with a positive reset. An added benefit of the Secure Action system is that it&#39;s easier to rack the slide, compared to a similar size striker-fired pistol.<br />"
    html = html + "<br />"
    html = html + "The pistol&#39;s frame is constructed with glass-filled nylon with a precision machined, hard-coat, aluminum chassis that contacts the slide with full-length guide rails. The slide is made of through-hardened alloy steel and blued; front and rear serrations provide a non-slip hold while loading or clearing the weapon. Front and rear sights are dovetailed to the slide, and drift adjustable.<br />"
    html = html + "<br />"
    html = html + "Sharp edges have been chamfered to aid in concealment, drawing the pistol and holstering. The Security-9 Compact Semi-Auto Pistol comes equipped with a trigger safety and a manual thumb safety. The slide stop serves as the takedown pin to easily fieldstrip the pistol for cleaning. Ships with 2 magazines, and 1 finger grip extension floorplate. Made in USA."
    html = html + "</p>"
    ...
    html = html + "Safety: Manual Safety<br />"
    html = html + "Frame Finish: Turquoise<br />"
    html = html + "Front Sight: Fixed<br />"
    html = html + "Rear Sight: Drift Adjustable<br />"
    html = html + "Grips: Textured<br />"
    html = html + "Features: Bladed Trigger SafetyFront Cocking Serrations<br />"
    html = html + "Slide Finish: Blue Hardened Alloy Steel<br />"
    html = html + "Finish: Blue<br />"
    html = html + "Magazine Count: 2.000<br />"
    html = html + "Firing System: Striker<br />"
    html = html + "Frame Material: Glass Filled Nylon<br />"
    html = html + "Barrel Configuration: Single<br />"
    html = html + "Rifled Barrel: Yes<br />"
    html = html + "Forend Rail: Picatinny Rail"
    html = html + "</div>"
    ...
    return "<p style='color:red'>Description</p>";
}
